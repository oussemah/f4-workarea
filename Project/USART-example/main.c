...
int main(void) {
    ...
    USART_puts(USART1, "\e[1;1H\e[2J\r\n"); // clear screen

    SPI_Config();

    USART_puts(USART1, "Started USART!\r\n");
    sd_st=SD_init();
    if(sd_st) {
        sprintf(buffer, "Err SD Init! code:%d\r\n\0", sd_st);
        USART_puts(USART1, buffer);
        //USART_putc(USART1, &sd_st);
    } else {
        sprintf(buffer, "SD inited! SDHC:%d\r\n\0", SDHC);
        USART_puts(USART1, buffer);
    }
